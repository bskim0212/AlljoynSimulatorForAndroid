...
public class PropertyMaker implements BusObject, CPPropertyInterface {
	...
	public PropertyMaker(InterfaceObject _obj, String _deviceId) {
		...
		// bskim 0922 추가 장비에 대한 하드코딩
		

		// if (isNumber(obj.getIf_default_value()) && obj.getIf_min_value() !=
		// null
		// && obj.getIf_max_value() != null) {
		//
		//
		// int imin = Integer.valueOf(obj.getIf_min_value());
		// int imax = Integer.valueOf(obj.getIf_max_value());
		...
		// for (int i = imin; i <= imax; i++) {
		// String label = "" + i;
		// Variant vValue = new Variant("" + i, obj.getIf_signal());
		//
		// if (obj.getIf_has_index() == 1 && obj.getInterface_range().size() >
		// 0) {
		// for (int k = 0; k < obj.getInterface_range().size(); k++) {
		// if (i ==
		// Integer.valueOf(obj.getInterface_range().get(k).getRange_index())) {
		// label = obj.getInterface_range().get(k).getRange_label();
		// }
		// }
		// }
	...
	public void setValue(Variant value) throws BusException {
		...
		if (isNumber(obj.getIf_default_value()) && !obj.getIf_signal().equals(BusMapObject.DATA_TYPE_B)
				&& obj.getIf_min_value() != null && obj.getIf_max_value() != null) {

			double test = 0;
			double testMin = Double.valueOf(obj.getIf_min_value());
			double testMax = Double.valueOf(obj.getIf_max_value());
			if (obj.getIf_signal().equals(BusMapObject.DATA_TYPE_D)) {
				test = value.getObject(Double.class);
			} else if (obj.getIf_signal().equals(BusMapObject.DATA_TYPE_S)) {
				String cValue = value.getObject(String.class);
				test = Double.valueOf(cValue);
			} else if (obj.getIf_signal().equals(BusMapObject.DATA_TYPE_Q)
					|| obj.getIf_signal().equals(BusMapObject.DATA_TYPE_N)) {
				test = value.getObject(Short.class);
			} else if (obj.getIf_signal().equals(BusMapObject.DATA_TYPE_X)
					|| obj.getIf_signal().equals(BusMapObject.DATA_TYPE_T)) {
				test = value.getObject(Long.class);
			} else if (obj.getIf_signal().equals(BusMapObject.DATA_TYPE_I)
					|| obj.getIf_signal().equals(BusMapObject.DATA_TYPE_U)) {
				test = value.getObject(Integer.class);
			}
	...
	public void ValueChanged(Variant newValue) throws BusException {
		...
		if (BusConnectionService.joinerInfo != null && !BusConnectionService.joinerInfo.isEmpty()
			...
			if (obj.getIf_noti_flag() == 1) {
				...
				} else if (iPath.contains("RefrigeratorIcePlusStatusValue") || iPath.contains("/RefSet/RefEnSet/RefEnSetIcePlus")) {
					Log.e("bskim", "qv=" +qv);
				...
				} else if (iPath.contains("RefrigeratorDoorOpenStatusValue") || iPath.contains("/RefSet/RefEnSet/RefEnSetDoorStatus")) {
					if (qv.equalsIgnoreCase("1") || qv.equalsIgnoreCase("Open") ) {
						msg = "Door Open";
